...
export function buildStepInfo(
  ...
  // 阈值定义为各档最大值：step2=第一档上限，step3=第二档上限。
  // 非法配置不参与计算，避免负区间和错误进度条。
  const auto = stepThresholds(mode)
  const rawStep2 = override?.step2 && override.step2 > 0 ? override.step2 : auto.step2
  const rawStep3 = override?.step3 && override.step3 > 0 ? override.step3 : auto.step3
  const step2 = Number.isFinite(rawStep2) && rawStep2 > 0 ? rawStep2 : auto.step2
  const fallbackStep3 = Math.max(auto.step3, step2 + 1)
  const step3 = Number.isFinite(rawStep3) && rawStep3 > step2 ? rawStep3 : fallbackStep3
...
export function buildViewModel(
  ...
  const currentMonthEle = Number.isFinite(account.haCurrentMonthUsage)
    ? round2(account.haCurrentMonthUsage as number)
    : sumCurrentMonth(dayElePq, monthElePq)
  ...
  const dayFee = Number.isFinite(account.haLatestDailyUsage)
    ? round2(account.haLatestDailyUsage as number)
    : dayElePq.length > 0 ? dayElePq[0].elePq : 0
